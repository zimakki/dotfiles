...
`zshenv` sources `hosts/<LocalHostName>.zsh` if it exists (runs for every shell,
so it's the environment layer — PATH, exports). Missing file = no-op, so a fresh
machine needs no bootstrap.
